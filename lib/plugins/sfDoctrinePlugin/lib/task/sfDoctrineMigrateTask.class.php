...
EOF;
    ...
    protected function execute($arguments = [], $options = [])
    {
        ...
        $this->logSection(
            'doctrine',
            sprintf(
                'Migrating from version %s to %s%s',
                $from,
                $version,
        ...
        try {
            ...
            if ($version < $from) {
                for ($i = (int)$from - 1; $i >= (int)$version; $i--) {
                    $this->logSection(
                        'doctrine',
                        'executing migration : ' . $i . ', class: ' . $migration_classes[$i]
                    );

                    $result = $migration->migrate($i, $options['dry-run']);

                    if ($result === false) {
                        $this->logSection('doctrine', 'migration error');
                    } else {
                        $this->logSection('doctrine', 'migrated to version: ' . $result);
                    }
                }
            } else {
                for ($i = (int)$from + 1; $i <= (int)$version; $i++) {
