...
class sfFormField
{
    protected static ?Exception $toStringException = null;

    protected ?sfWidgetForm $widget = null;
    protected ?sfFormField  $parent = null;
     ...
     * @param string|array|null     $value  The field value
     ...
     * @return bool
     */
    public static function hasToStringException() : bool
    {
     ...
     * @return Exception
     */
    public static function getToStringException() : ?Exception
    {
     ...
     */
    public function renderRow($attributes = [], $label = null, $help = null) : string
    {
        if (null === $this->parent) {
            throw new LogicException(sprintf('Unable to render the row for "%s".', $this->name));
        }

        $field = $this->parent->getWidget()->renderField(
            $this->name,
            $this->value,
        ...
        return strtr(
            $this->parent->getWidget()->getFormFormatter()->formatRow(
    ...
    public function renderError() : string
    {
        ...
        $error = $this->getWidget() instanceof sfWidgetFormSchema
            ? $this->getWidget()->getGlobalErrors($this->error)
            : $this->error;
    ...
    public function renderHelp() : string
    {
        ...
        return $this->parent->getWidget()->getFormFormatter()->formatHelp(
            $this->parent->getWidget()->getHelp($this->name)
        );
    }
     ...
     */
    public function renderLabel($label = null, $attributes = []) : string
    {
        if (null === $this->parent) {
     ...
     */
    public function renderLabelName() : string
    {
        if (null === $this->parent) {
    ...
    {
        return $this->widget->generateId(
            $this->parent ? $this->parent->getWidget()->generateName($this->name) : $this->name,
            $this->value
        );
    }
     ...
     */
    public function getParent() : ?sfFormField
    {
        return $this->parent;
    }
